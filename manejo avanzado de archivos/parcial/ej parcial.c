...
int main()
{
    ...
    while (strcmpi(esp,"fin")!=0)
    {
       do
       {
        ...
        rewind(pf); // posisino al principio del puntero
        band=0;
        ...
        while (band == 0 && !feof(pf))
        {
            ...
            {
                band = 1; // se encontro la especie
            }
            else
                fread(&siembra,sizeof(plantas),1,pf);
            
            if(band == 0) // se crea ua nueva especie si es el primero ya que el archivo esta vacio
            {
                strcpy(siembra.especie,esp);  //copio y creo la nueva especie
                siembra.est=1;
                siembra.hec = cant;

                fflush(pf);

                fwrite(&siembra,sizeof(plantas),1,pf); //la grabo en el archivo
            }
            else // si la especie no es nueva tengo que agregar los nuevos datos
            {
                siembra.est++; //aumento los establecimientos
                siembra.hec+=cant; // aumento las hectareas
                fseek(pf,sizeof(plantas)*(-1),1); // muevo el puntero uno para atras
                fwrite(&siembra,sizeof(plantas),1,pf); // grabo la nuyeva info sobre la anterior
    ...
    rewind(pf);
    fread(&sembrado,sizeof(sembrado),1,pf);
    band = 0;
    while (!feof(pf))  //inforamcion general del ej
    {
        band++; //aprovecho para poder contar la cantidad de especies que hay en la zona
    ...
    printf("\n hay un total de %d especies en la zona",band);
    fclose(pf);
    printf("\n ingrese nombre para el archivo: ");
    fflush(stdin);
    gets(esp); //resiclo la variable de especie
    nombre(esp);
    return 0;
}
...
void mostrar()
{
    ...
    while (!feof(pf))  //inforamcion general del ej
    {
        especies++; //aprovecho para poder contar la cantidad de especies que hay en la zona
        printf("\nel cultivo %20s tiene un total de %d establecimientos y un total de %.2f de hectareas",sembrado.especie,sembrado.est,sembrado.hec);
        fread(&sembrado,sizeof(plantas),1,pf);
    }
...
{
    if( rename("simbra.dat",nombrenuevo)==0) // se cambia el nobre
        printf("\nombre cambiado");
    else    
        printf("\nno se pdo cambiar el nombre"); // no se pudo cambiar el nombre
